...
var pluginSubcommands = map[string]bool{
	...
	// doctor has no real Python equivalent — a Go-only addition per the
	// migration spec ("Replace check-plugins.py with a native
	// non-destructive command such as `pwnagotchi plugins doctor --all
	// --no-hardware`"). --all/--no-hardware are accepted (matching that
	// suggested invocation) but currently no-ops: every check this
	// subcommand runs is already non-hardware/non-destructive by design,
	// there is no narrower/hardware-touching mode yet to opt out of.
	"doctor": true,
}
