...
3. `tests/visual/compat_live_test.go` (build tag `compatibility`) re-runs
   `oracle.py` against a live installed Python package and byte-diffs it
   against the checked-in golden, so the golden itself can be checked for
   drift against real Python behavior. **Disclosed staleness**: the
   `make compatibility-test` Makefile target this file's own comments
   reference was removed elsewhere in this migration (all other Python
   differential-test infrastructure was deleted once the plugin bridge
   went away); this one file was deliberately left as-is because it is
   display/visual-rendering-adjacent test infrastructure, out of scope for
   the current phase of this migration by user request (see the root
   `README.md`). It still compiles and is still gated behind the
   `compatibility` build tag (never part of plain `go test ./...`), so its
   staleness is inert, not a live instruction to install Python — but
   running it today requires manually setting `PWNAGOTCHI_PYTHON` and
   invoking `go test -tags=compatibility ./tests/visual/...` directly,
   not `make compatibility-test`.
...
entirely inside the Go module (originally `go-port/`, since restructured
to the repository root — see `docs/migration-ledger.md`). No UI layout,
font asset, or native resolution was redesigned; the existing per-driver
layouts (`hw_layouts.json`) and embedded DejaVu Sans Mono files are
unchanged.
...
PWNAGOTCHI_PYTHON=/path/to/python3 \
  go test -tags=compatibility ./tests/visual/...    # also regenerates + byte-diffs the golden against live Python
                                                     # (the old `make compatibility-test` target no longer exists —
                                                     # see this document's note above)
```
...
```bash
# run from the repository root (paths below assume a real Python venv with
# the pwnagotchi package installed exists at ./venv — not set up in this repo today)
