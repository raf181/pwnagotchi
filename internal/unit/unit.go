...
// ReadCPUStat returns the split, integer-parsed first line of /proc/stat
// (the raw per-mode jiffy counters: user, nice, sys, idle, iowait, irq,
// softirq, steal, guest, guest_nice), exported so callers with their own
// independent load-averaging window (e.g. a plugin tracking its own
// last-sample instead of pwnagotchi.cpu_load()'s tag-cache/sleep
// semantics — see internal/plugins/native/memtemp) can diff two samples
// themselves without reimplementing /proc/stat parsing.
