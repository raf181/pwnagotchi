...
// DefaultRunRoot is the production runtime root under which disk-backed
// staging directories for memory-fs mounts are created. Tests must inject
// a t.TempDir() via SetupMounts' runRoot parameter instead of relying on
// this so they remain hermetic and require no root privileges.
const DefaultRunRoot = "/run/pwnagotchi"
...
func SetupMounts(r Runner, enabled bool, mounts map[string]MountConfig, stop <-chan struct{}, runRoot string) ([]*MemoryFS, error) {
	if !enabled {
		return nil, nil
	}
	if runRoot == "" {
	...
	for _, options := range mounts {
		...
		target := filepath.Join(runRoot, "disk", filepath.Base(options.Mount))
