...
> snapshot of behavior captured by running the real Python interpreter
> earlier in the migration — it is NOT an instruction to set up a Python
> venv today. `internal/pyplugin`, `internal/plugins/loader.go`,
> `PWNAGOTCHI_PYTHON`, and the `compatibility-test` Makefile target
> (which required this venv) have all been deleted; nothing in the
> current build/test/CI path requires Python. A full rewrite (task #13
> of the migration) is still pending.
...
(mirrored as fixtures under `testdata/`).
...
Because there is nothing to run, `tests/` characterization tests
(Go) and the fixtures under `testdata/` are the concrete, executable
