...
type Server struct {
	...
	agent     AgentInfo
	grid      *grid.Client
	pluginMgr *pluginmanager.Manager // every plugin is native; this is the only plugin backend
	actions   Actions
	cfg       config.Map
	cfgPath   string
...
func New(cfg config.Map, name string, agentInfo AgentInfo, gridClient *grid.Client, pluginMgr *pluginmanager.Manager, actions Actions, fullCfg config.Map, cfgPath string) *Server {
