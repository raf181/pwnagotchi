...
import (
	...
	"github.com/jayofelony/pwnagotchi/internal/ui/components"
	...
	"github.com/jayofelony/pwnagotchi/internal/ui/state"
...
// Kind reports the resolved display driver's registry name (e.g.
// "waveshare_2", "waveshare144lcd", "" if headless/no display) — the
// same string display.Display's generated IsWaveshareV2()-style
// predicates compare against (see internal/ui/display/is_methods_gen.go).
// Exposed on View itself (not just Display) because plugins' on_ui_setup/
// on_ui_update/on_unload hooks receive whatever View the daemon is
// running with the display-type check inlined directly here, rather than
// needing every one of the ~90 generated is_X() predicates duplicated on
// a plugin-facing capability interface.
...
// resolveFont maps a case-insensitive font role name (matching
// pwnagotchi/ui/fonts.py's module attribute names: Small, Bold,
// BoldSmall, Medium, Huge, BoldBig) to this View's actual font faces
// (base + supersampled), so plugin-added elements render with the exact
// same font metrics/anti-aliasing pipeline as built-in elements. An
// unrecognized or empty name falls back to Small, matching "just show
// something legible" over a hard error for a cosmetic mistake in a
// plugin's own config.
func (v *View) resolveFont(name string) (face, superFace xfont.Face) {
	set, superSet := v.fonts, v.fontsSuper
	switch strings.ToLower(name) {
	case "bold":
		return set.Bold, superSet.Bold
	case "boldsmall":
		return set.BoldSmall, superSet.BoldSmall
	case "boldbig":
		return set.BoldBig, superSet.BoldBig
	case "medium":
		return set.Medium, superSet.Medium
	case "huge":
		return set.Huge, superSet.Huge
	default:
		return set.Small, superSet.Small
	}
}

// AddTextElement ports `ui.add_element(key, Text(...))` for the plain
// (unlabeled) Text widget shape used by memtemp/session-stats/ups_lite/
// wittypi/pisugarx and most other bundled plugins' on_ui_setup. font is a
// case-insensitive font role name (see resolveFont); wrap/maxLength mirror
// components.Text.Wrap/MaxLength (0 disables wrapping).
func (v *View) AddTextElement(key, value string, x, y int, font string, wrap bool, maxLength int) {
	face, superFace := v.resolveFont(font)
	v.mu.Lock()
	color := v.black
	v.mu.Unlock()
	w := &components.Text{
		Value:     value,
		Position:  components.Point{X: x, Y: y},
		Font:      face,
		SuperFont: superFace,
		Color:     color,
		Wrap:      wrap,
	...
	v.state.AddElement(key, newTextElement(w, &value))
}

// AddLabeledValueElement ports `ui.add_element(key, LabeledValue(...))`,
// the label+value widget shape used by session-stats/memtemp's vertical
// layout/ups_lite and others. An empty label matches Python's
// `label=None` (value-only rendering, no leading label text).
func (v *View) AddLabeledValueElement(key, label, value string, x, y int, labelFont, valueFont string, labelSpacing int) {
	lFace, lSuperFace := v.resolveFont(labelFont)
	vFace, vSuperFace := v.resolveFont(valueFont)
	v.mu.Lock()
	color := v.black
	v.mu.Unlock()
	w := &components.LabeledValue{
		...
		LabelFont:      lFace,
		TextFont:       vFace,
		LabelFontSuper: lSuperFace,
		TextFontSuper:  vSuperFace,
		Color:          color,
	...
	v.state.AddElement(key, &labeledValueElement{widget: w})
}
