...
that the Go port (`go-port/`) must account for. It was produced by direct inspection of
...
There are **zero existing automated tests** anywhere in the repository (`find . -iname
"*test*"` returns nothing outside `venv/`). Python is still the behavioral oracle per
the porting goal, so `go-port/docs/python-baseline.md` captures characterization
