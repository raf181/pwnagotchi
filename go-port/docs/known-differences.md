...
never `x509.MarshalPKCS1PublicKey`), confirmed byte-for-byte against a real
pycryptodome-generated key in `tests/compat_identity_test.go`
(`TestCompatFingerprintMatchesPython`) — same SHA-256 fingerprint from the
same key material.
...
into `go-port/internal/voice/locale/` (verbatim, unmodified bytes) and
...
tracked outstanding work — see `docs/final-port-report.md` — not a silent
gap.

### `internal/pyplugin` bridge: `agent`/`view`/`display` arguments are stubs, not live proxies
Real `pwnagotchi.plugins.on(event, *args)` runs inside the same Python
process as the rest of the daemon, so a plugin's `on_wifi_update(self,
agent, access_points)` can call real methods on `agent` (e.g.
`agent.view()`, `agent.session()`). The Go port's plugins run in a
*separate* Python process (see `internal/pyplugin/bridge.py`), so any
argument that isn't plain JSON-safe data (maps/lists/strings/numbers) —
concretely, every `*agent.Agent`/`view.View`/`hw.Driver` argument — is
replaced with a `{"__goref__": "<TypeName>"}` marker, turned into a
`_GoProxyStub` on the Python side. Calling *any* method or reading any
attribute on that stub raises a real, clearly-worded `NotImplementedError`
(not a silent no-op, not fake success). Verified real-world impact: bundled
`cache.py` never touches its `agent`/`ui` arguments and runs identically to
real Python (see `tests/compat_pyplugin_test.go`); a plugin that DID call
`agent.something()` would get a loud, real failure logged to bridge
stderr instead of the real behavior. Building a full bidirectional RPC
proxy for these objects (so stub method calls actually round-trip to the Go
...
real hardware (tracked as unverified-on-CI in `final-port-report.md`).
...
### Plugin webhook/toggle bridge calls: synchronous RPC added to `internal/pyplugin`
`internal/web`'s `/plugins` routes need real answers from real Python
`plugins.py` state that the original fire-and-forget `On(event, args...)`
protocol can't provide (a page to render, a boolean toggle result, an
HTTP response body). `internal/pyplugin/bridge.py` was extended with a
second, synchronous message kind (`"call"` vs. the original `"event"`) —
see its module docstring — used only for `list_plugins`, `toggle_plugin`,
and `webhook`. `toggle_plugin`'s "enable" path in real Python
unconditionally reads `view.ROOT._agent` (crashes with a real
`AttributeError` in a bare bridge process, since `view.ROOT` is never set
there); the bridge sets `pwnagotchi.ui.view.ROOT` to a minimal stand-in
object with `_agent = None` before calling it — the same state a real
...
See `docs/final-port-report.md` for the full list of `internal/ui/hw/*`
...
support isn't present on the build/test machine.

**Update:** this build/test machine does have a real MediaTek MT7612U USB
Wi-Fi adapter and a real running `bettercap` instance, so two items are no
longer in this bucket: `internal/config.IfaceChannels` and
`internal/bettercap.Client.Session()` are now verified against real
hardware/services (`tests/live_hardware_test.go`, `-tags=live`, opt-in
only). No `ui/hw` display driver has real e-ink/OLED hardware to verify
against in this rig, so those remain unverified. See
`docs/final-port-report.md`'s incident note on why the live-hardware suite
deliberately never touches `internal/unit.SetName`/`Restart`/`Reboot`.
